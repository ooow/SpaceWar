...
class GameScene: SKScene, SKPhysicsContactDelegate {
  ...
  override func didMove(to view: SKView) {
    ...
    gameTimer = Timer.scheduledTimer(timeInterval: aliensTimeInterval,
        target: self, selector: #selector(addAlien),
        userInfo: nil, repeats: true)
    motionManager.accelerometerUpdateInterval = accelerometerUpdateInterval
    motionManager.startAccelerometerUpdates(to: OperationQueue.current!) {
      (date: CMAccelerometerData?, error: Error?) in
      if let accelerometerDate = date {
        let acceleration = accelerometerDate.acceleration
        self.xAccelerate = CGFloat(acceleration.x) * 0.75
            + self.xAccelerate * 0.25
      }
    }
  }
  ...
  @objc func addAlien() {
    aliens = GKRandomSource.sharedRandom().arrayByShufflingObjects(in: aliens)
        as! [String]
    ...
    actions.append(SKAction.move(to: CGPoint(x: position,
        y: -alien.size.height), duration: animationDuration))
    // Remove the alien after going out of the screen.
